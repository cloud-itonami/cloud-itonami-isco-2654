(ns video-production.actor
  "VideoOpsActor — the ISCO-08 2654 independent video director/producer
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one production operation request (intake →
  ...
  The unconditional invariant: the VideoOpsAdvisor can never directly commit
  a record the VideoProductionGovernor refuses — every commit-record! call
  is gated behind `:decide`, and assemble commits carry the actual douga
  render plan (kotoba-lang/douga) so what was approved is exactly what
  renders."
  (:require [douga.ffmpeg :as ffmpeg]
            [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [video-production.advisor :as advisor]
            [video-production.governor :as governor]
            [video-production.store :as store]))

(defn build-graph
  "Build a compiled VideoOpsActor graph. `store` implements
  `video-production.store/Store`. `advisor` implements
  `video-production.advisor/Advisor` (defaults to `mock-advisor`).
